...
public class Board extends JPanel implements KeyListener {
	...
	public void update() {
		...
		if(gameOver) {
			timer.stop();
			JOptionPane.showMessageDialog(null, "GAME OVER", "Info", JOptionPane.PLAIN_MESSAGE);
		}
	}
	...
	public void paintComponent(Graphics gr) {
		...
		for(int row = 0; row < board.length; row++) {
			for(int col = 0; col < board[row].length; col++) {
				if(board[row][col] != 0) {
					gr.drawImage(blocks.getSubimage((board[row][col] - 1) * blockSize, 0, blockSize, blockSize), 
	...
	public void keyPressed(KeyEvent e) {
		...
		if(e.getKeyCode() == KeyEvent.VK_P) {
			...
			if(counterPause == 1) {
				timer.stop();
				JOptionPane.showMessageDialog(null, "The Game is paused", "Info", JOptionPane.PLAIN_MESSAGE);
			}
